...
def get_coin_logo_url(coin_id):
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            return data.get('image', {}).get('thumb', None) or data.get('image', {}).get('small', None)
    except Exception:
        pass
    return "https://static.thenounproject.com/png/3416924-200.png"  # fallback
...
st.markdown("<h2 class='coin-section'>Top 5 Coinswitch Coins Predicted to be Green (Next 5 Days)</h2>", unsafe_allow_html=True)
if coinswitch_df is not None and not coinswitch_df.empty:
    for idx, row in coinswitch_df.iterrows():
        ...
        st.markdown(f"<div class='coin-section'><a href='#coin_{coin_id}' class='coin-link'><img src='{logo_url}' width='24' class='coin-logo'/> <b>{row['Coin']}</b></a> - {row['5-Day % Change']}", unsafe_allow_html=True)
        # Show next 5 days price and % change (no graph here)
        try:
            ...
            st.markdown("<small>Next 5 days % change: " + ', '.join(pct_strs) + "</small>", unsafe_allow_html=True)
        except Exception:
            st.markdown(f"<small>Next 5 days: {row['5-Day Predicted Prices']}</small>", unsafe_allow_html=True)
...
if results_df is not None and not results_df.empty:
    ...
    for idx, row in top10.iterrows():
        ...
        logo_url = get_coin_logo_url(coin_id)
        st.markdown(f"<div class='coin-section'><a href='#coin_{coin_id}' class='coin-link'><img src='{logo_url}' width='24' class='coin-logo'/> <b>{row['Coin']}</b></a> - {row['5-Day % Change']}", unsafe_allow_html=True)
        # Show next 5 days % change (no graph here)
        try:
            prices = eval(row['5-Day Predicted Prices']) if isinstance(row['5-Day Predicted Prices'], str) else row['5-Day Predicted Prices']
            pct_changes = [None]
            for i in range(1, len(prices)):
...
if results_df is not None and not results_df.empty:
    for idx, row in results_df.iterrows():
        ...
        try:
            ...
            st.markdown("<small>Price Change % (Day 2-5):</small>")
            if len(prices) > 1:
